...
│   │   │   ├── IterativeSystem.java
│   │   │   ├── ConvergenceTest.java
│   │   │   └── Solver.java                 # (es. Interfaccia per un risolutore numerico)
│   │   ├── fractals/                       # Contiene implementazioni specifiche di frattali
│   │   │   ├── MandelbrotEvaluator.java
│   │   │   └── JuliaEvaluator.java
...
// Interfaccia per un test di convergenza generico (utile ovunque)
@FunctionalInterface
public interface ConvergenceTest<T> {
    // Restituisce true se la sequenza è convergente entro una certa tolleranza
    boolean isConverged(T current, T previous, Real tolerance);
}

// Interfaccia per qualsiasi sistema che evolve iterativamente (es. metodi di Newton, Runge-Kutta)
public interface IterativeSystem<T> {
    T nextIteration(T current);
}

// Interfaccia generica per un solutore numerico
public interface Solver<P, S> { // P=Problema, S=Soluzione
    S solve(P problem, ConvergenceTest<S> test);
}


per i polinomi:
PolynomialRing<E extends FieldElement<E>>.
...
public class MandelbrotEvaluator implements IterativeSystem<Complex> {
    private final Complex c; // Il punto fisso per questa particolare valutazione

    public MandelbrotEvaluator(Complex c) { this.c = c; }

    @Override
    public Complex nextIteration(Complex z_n) {
        // z_{n+1} = z_n^2 + c
        return z_n.multiply(z_n).add(c);
    }
}
