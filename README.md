...
public final boolean equals(Object o) 
{
    ...
    public boolean isApproximatelyEqualTo(RealMatrix other, double epsilon) {
        // 1. Controlli preliminari (null, identit‡, dimensioni)
        if (this == other) {
            return true;
        }
...
- Suggerimento: Vincola l'interfaccia HermitianOperator in modo pi˘ stretto, non solo a VectorElement, ma a InnerProductSpaceElement.
// Vincolo pi˘ stretto per i problemi di MQ:
public interface HermitianOperator<K extends FieldElement<K> & NormableElement<Real, K>, 
    ...
    Real expectationValue(V state); // Funziona solo se il prodotto scalare Ë definito
}
