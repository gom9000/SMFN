...
implements Field<Rational>, ExactStructure<Rational>, NumericFactory<Rational>
{
    ...
    public Rational of(double value) {
        ...
        // Se l'esponente � 0, � un valore denormalizzato. Altrimenti � normalizzato.
        if (exponent == 0) { 
            // Denormalizzato: l'esponente � -1022, il bit implicito non � aggiunto
            exponent = -1022; 
        } else {
            // Normalizzato: aggiunge il bit implicito (il 53� bit della mantissa)
            mantissa |= 0x0010000000000000L; 
            // Il bias dell'esponente � 1023
            exponent -= 1023;
        }

        // Il valore �: mantissa * 2^exponent
