...
implements Semimodule<Matrix<K>, K, S>, CompositeElementFactory<Matrix<K>, K[]>
{
    ...
    public Matrix<K> of(K[] data) {
    	if (data == null || data.length != rows * cols) {
            throw new IllegalArgumentException("Matrix dimensions mismatch. Expected " + (rows * cols) + " elements (" + rows + "x" + cols + "), got " + (data == null ? "null" : data.length));
        }
    ...
    public boolean contains(Matrix<K> m) {
        return m != null
            && m.getRows() == this.rows && m.getCols() == this.cols
